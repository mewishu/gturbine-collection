...
class JarFileEntries implements CentralDirectoryVisitor, Iterable<JarEntry> {
    ...
    private <T extends FileHeader> T getEntry(CharSequence name, Class<T> type, boolean cacheEntry) {
        ...
        if (!isMetaInfEntry(name) && isMultiReleaseJar()) {
            ...
            AsciiBytes nameAlias = (entry instanceof JarEntry) ? ((JarEntry) entry)
                .getAsciiBytesName() : new AsciiBytes(name.toString());
            while (version > BASE_VERSION) {
                T versionedEntry = doGetEntry("META-INF/versions/" + version + "/" + name, type,
                    cacheEntry, nameAlias);
    ...
    private <T extends FileHeader> T getEntry(int hashCode, CharSequence name, char suffix,
