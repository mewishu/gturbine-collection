 ...
 */
package org.excellentcoder.gturbine.archive.jar;

import java.io.IOException;
import java.time.LocalDateTime;
import java.time.ZoneId;

import org.excellentcoder.gturbine.archive.data.RandomAccessData;
...
final class CentralDirectoryFileHeader implements FileHeader {
    ...
    CentralDirectoryFileHeader(byte[] header, int headerOffset, AsciiBytes name, byte[] extra,
                               AsciiBytes comment, long localHeaderOffset) {
        this.header = header;
    ...
    void load(byte[] data, int dataOffset, RandomAccessData variableData, int variableOffset,
    ...
    private long decodeMsDosFormatDateTime(long datetime) {
        LocalDateTime localDateTime = LocalDateTime.of((int) (((datetime >> 25) & 0x7f) + 1980),
            ...
            (int) ((datetime >> 11) & 0x1f), (int) ((datetime >> 5) & 0x3f),
            (int) ((datetime << 1) & 0x3e));
        return localDateTime.toEpochSecond(ZoneId.systemDefault().getRules()
            .getOffset(localDateTime)) * 1000;
    }
    ...
    static CentralDirectoryFileHeader fromRandomAccessData(RandomAccessData data, int offset,
