 ...
 */
package org.excellentcoder.gturbine.maven;
...
import org.excellentcoder.gturbine.loader.tools.DefaultLaunchScript;
import org.excellentcoder.gturbine.loader.tools.LaunchScript;
import org.excellentcoder.gturbine.loader.tools.Layout;
import org.excellentcoder.gturbine.loader.tools.LayoutFactory;
import org.excellentcoder.gturbine.loader.tools.Layouts.Expanded;
import org.excellentcoder.gturbine.loader.tools.Layouts.Jar;
import org.excellentcoder.gturbine.loader.tools.Layouts.None;
import org.excellentcoder.gturbine.loader.tools.Layouts.War;
import org.excellentcoder.gturbine.loader.tools.Libraries;
import org.excellentcoder.gturbine.loader.tools.Repackager;
import org.excellentcoder.gturbine.loader.tools.Repackager.MainClassTimeoutWarningListener;

/**
 * gturbine-maven-plugin打包入口:
 *
...
public class RepackageMojo extends AbstractDependencyFilterMojo {
        ...
        if (classifier != null) {
            for (Artifact attachedArtifact : this.project.getAttachedArtifacts()) {
                if (classifier.equals(attachedArtifact.getClassifier())
                    && attachedArtifact.getFile() != null && attachedArtifact.getFile().isFile()) {
    ...
    private String removeLineBreaks(String description) {
        return (description != null) ? WHITE_SPACE_PATTERN.matcher(description).replaceAll(" ")
            : null;
    }
    ...
    private void updateArtifact(Artifact source, File target, File original) {
        if (this.attach) {
            ...
            String artifactId = (this.classifier != null) ? "artifact with classifier "
                                                            + this.classifier : "main artifact";
            getLog().info(
                String.format("Updating %s %s to %s", artifactId, source.getFile(), original));
            source.setFile(original);
        } else if (this.classifier != null) {
            getLog().info(
                "Creating repackaged archive " + target + " with classifier " + this.classifier);
        }
    }
    ...
    private void attachArtifact(Artifact source, File target) {
        if (this.classifier != null && !source.getFile().equals(target)) {
            getLog().info(
                "Attaching repackaged archive " + target + " with classifier " + this.classifier);
